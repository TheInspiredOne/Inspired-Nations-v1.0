...
public class Point3D implements Serializable, Cloneable {
	...
	public Point3D(Location location, ClientID server) {
	...
	public Point3D(int x, int y, int z, World world, ClientID server) {
    ...
    public ClientLocationPortalInter getLocation() throws RemoteException, NotBoundException {
    	return world.getClient().getClientPortal().getLocation(this);
    
    	//return new Location(InspiredNations.plugin.getServer().getWorld(this.world.toString()), x, y, z);
    }
	public double distance(Point3D location) {
